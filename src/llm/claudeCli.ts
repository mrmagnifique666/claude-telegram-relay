...
import { getTurns, type Turn } from "../storage/store.js";
import { getToolCatalogPrompt } from "../skills/loader.js";

const SYSTEM_POLICY = `You are a helpful assistant operating through a Telegram relay.
Be concise. Respond in plain text unless the user asks for code or formatting.
 ...
 */
function buildPrompt(chatId: number, userMessage: string): string {
  const parts: string[] = [];
...
export async function runClaude(
  ...
  const prompt = buildPrompt(chatId, userMessage);
